...
def write_inlist(desired_mass, desired_Z, desired_abund, dir_to_write):
    ...
    abund_str = ['!INITIAL_H1!', '!INITIAL_H2!', '!INITIAL_HE3!',
            '!INITIAL_HE4!']
    assert desired_mass > 0.35, 'Numerically unstable below desired stellar'+\
        'mass when using solar fine-tuned mass frac (Choi+ 2016 Table 2)'
    desired_BC = 'photosphere_tables' if desired_mass >= 0.6 \
        else 'tau_100_tables'
    ...
    for ix, l in enumerate(lines):
        if ('!BC_TABLE!' in l):
            existing_BC = l.split('\'')[1]
            lines[ix] = l.replace(existing_BC, desired_BC)
    for ix, l in enumerate(lines):
        if ('INITIAL_Z' in l):
...
def make_sym_links(mass, Z, ini_abunds, dir_to_write):
    ...
    os.symlink(MESA_BASE+'/inlist_to_run', MESA_RUN+'/'+dir_to_write+'inlist_to_run')

def make_dir_loop(mass_grid, Z_grid, k, solar_flag):
    for mass in mass_grid:
        for Z in Z_grid:
            ...
            make_sym_links(float(mass), float(Z), ini_abunds, dir_to_write)
            k += 1
    return k

if __name__ == '__main__':
    solar_flag = True # required for special solar calibration.
    Z_solar = 0.0150 if solar_flag else 0.0142

    #1. Fix the mass to one of the grid masses, but select some number (say 20
    #   or so) random metallicities between -1 and +0.5.
    np.random.seed(0)
    min_mass, max_mass, mass_step = 0.8, 0.8, 0.1
    Z_grid = 10**np.random.uniform(low=-1., high=0.5, size=20)
    mass_grid = np.arange(min_mass, max_mass+mass_step, mass_step)
    Z_grid = 10**np.random.uniform(low=-1., high=0.5, size=20)*Z_solar

    k = 1
    k = make_dir_loop(mass_grid, Z_grid, k, solar_flag)

    #2. Same as above but mass and metallicity reversed.
    Z_grid = np.array([1.])*Z_solar
    mass_grid = np.random.uniform(low=0.4, high=1.2, size=20)
    k = make_dir_loop(mass_grid, Z_grid, k, solar_flag)

    #3. Some set with random masses and metallicities.
    Z_grid = 10**np.random.uniform(low=-1., high=0.5, size=4)*Z_solar
    mass_grid = np.random.uniform(low=0.4, high=1.2, size=4)
    k = make_dir_loop(mass_grid, Z_grid, k, solar_flag)
